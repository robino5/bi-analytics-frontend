...
export default function GsecTurnoverComparisonChart({
  ...
  if (!hasData) {
    return (
      <Card className={cn("w-full shadow-md", className, "bg-[#033e4a]")}>
        ...
        -Turnover-${numberToMillionsString(details?.sumOfTurnover || 0)}
         (${((details?.sumOfTurnover / (details?.sumOfTurnoverGsec + details?.sumOfTurnover)) * 100 || 0).toFixed(2)}%)
          And`} <span className="text-red-700 font-bold">{`GSEC-${numberToMillionsString(details?.sumOfTurnoverGsec || 0)}(${((details?.sumOfTurnoverGsec / details?.sumOfTurnover) * 100 || 0).toFixed(2)}%)`}</span></CardTitle>
        </CardHeader>
  ...
  return (
    <Card className={cn("w-full shadow-md", className, "bg-[#033e4a]")}>
      <CardHeader className="bg-gradient-to-r from-teal-900 via-teal-600 to-teal-8 00 p-2 rounded-tl-lg rounded-tr-lg">
        <CardTitle className="text-white text-md text-lg">{`${title}
