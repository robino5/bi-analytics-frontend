...
class BranchPerformanceAPI extends Common {
  ...
  getTurnoverStatus(branch: string) {
    if (branch) {
      return this.http.get<IResponse<ITurnoverStatus[]>>(
        `dashboards/branchwise-turnover-status/?branch=${branch}`,
      );
    } else {
      return this.http.get<IResponse<ITurnoverStatus[]>>(
        "dashboards/branchwise-turnover-status/",
      );
    }
  }
  getBranchWiseFundStatus(branch: string) {
    if (branch) {
      return this.http.get<IResponse<IBranchWiseFund[]>>(
        `dashboards/branchwise-fund-status/?branch=${branch}`,
      );
    } else {
      return this.http.get<IResponse<IBranchWiseFund[]>>(
        "dashboards/branchwise-fund-status/",
      );
    }
  }
  getBranchWiseMarginStatus(branch: string) {
    if (branch) {
      return this.http.get<IResponse<IBranchWiseMargin[]>>(
        `dashboards/branchwise-margin-status/?branch=${branch}`,
      );
    } else {
      return this.http.get<IResponse<IBranchWiseMargin[]>>(
        "dashboards/branchwise-margin-status/",
      );
    }
  }
  ...
  getBranchWiseExposureStatus(branch: string) {
    if (branch) {
      return this.http.get<IResponse<IBranchWiseExposure[]>>(
        `dashboards/branchwise-exposure-status/?branch=${branch}`,
      );
    } else {
      return this.http.get<IResponse<IBranchWiseExposure[]>>(
        "dashboards/branchwise-exposure-status/",
      );
    }
  }
  getDailyTurnoverPerformance(branch: string) {
    if (branch) {
      return this.http.get<IResponse<ITargetGenerated[]>>(
        `dashboards/daily-trade-performance/?branch=${branch}`,
      );
    } else {
      return this.http.get<IResponse<ITargetGenerated[]>>(
        "dashboards/daily-trade-performance/",
      );
    }
  }
